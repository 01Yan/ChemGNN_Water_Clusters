...
class MyDataset(InMemoryDataset):
    ...
    def process(self):
        pass
        # for i in range(6, 7):
        #     datasets = generate_dataset(f"data/1.8newdata/{i}WATER", "dataset/waterforce2/", config_data)

            # datasets = generate_dataset("data/DATA 1/", "dataset/waterforce2/", config_data, True)
            # datasets = generate_dataset("data/Full_data/", "dataset/waterforce2/", config_data)
            # datasets = generate_dataset("data/Full_test_data/", "dataset/waterforce2/", config_data)
        ...
        #
            ...
            #
            #         # 创建每个节点的邻居列表字典
            #         neighbors_dict = {i: set() for i in range(torch.max(data.edge_index) + 1)}
            #         for edge in data.edge_index.t().tolist():
            #             neighbors_dict[edge[0]].add(edge[1])
            #             neighbors_dict[edge[1]].add(edge[0])
            #
            #         # 计算共享邻居
            #         shared_neighbors_counts = []
            #
            #         # 使用itertools.combinations生成所有可能的节点对
            #         for u, v in itertools.combinations(range(torch.max(data.edge_index) + 1), 2):
            #             # 计算共享邻居数量
            #             shared_neighbors = neighbors_dict[u].intersection(neighbors_dict[v])
            #             shared_neighbors_counts.append(len(shared_neighbors))
            #
            #         # 计算平均共享邻居数量
            #         avg_shared_neighbors = sum(shared_neighbors_counts) / (torch.max(data.edge_index) + 1)
            #         shared_neighbor += avg_shared_neighbors.item()
            #         print(f"Average shared neighbors: {avg_shared_neighbors}")
            #
            #         edge_attr = adj[edge_index[0], edge_index[1]].to(torch.long)
            ...
            #     pbar.update(1)
            #
            # shared_neighbor = shared_neighbor / 5600
            # print(shared_neighbor)
            # pbar.close()
            #
            # # 保存数据为 .pt 文件（使用 PyTorch 的 torch.save() 函数）
            # torch.save(self.collate(data_list_pt), osp.join(self.processed_dir, f'Random_1water_newcutoff.pt'))
if __name__ == '__main__':
    train_dataset = MyDataset('/Users/yanhongyu/Documents/课程资料/summer_project/soap_1-21-water_energy/1/dataset/waterforce2')
